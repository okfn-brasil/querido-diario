...
Os comandos abaixo preparam o ambiente em sistema operacional Linux. Eles consistem em criar um [ambiente virtual de Python](https://docs.python.org/pt-br/3/library/venv.html), instalar os requisitos listados em `pyproject.toml` e a ferramenta para padronização de código `pre-commit`.
...
1. Vá para o diretório `querido_diario`:
```console
cd querido_diario
```
2. Acione o template:
...
source .venv/bin/activate
```
2. Vá para o diretório `querido_diario`:
```console
cd querido_diario
```
...
```
5. Os diários coletados na raspagem serão salvos no diretório `querido_diario/data`
