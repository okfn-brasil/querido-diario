...
class SpBauruSpider(Spider):
    ...
    def parse_year(self, response):
        ...
        months_links = response.xpath(f"{self.BASE_XPATH}"
                                      "/ul/li/ul/li/a/@href").extract()

        for month_link in months_links:
            yield Request(url=f"https://www2.bauru.sp.gov.br/juridico/{month_link}",
                          callback=self.parse_month)

    def parse_month(self, response):
        gazettes_seletor = response.xpath(f"{self.BASE_XPATH}"
                                          "/ul/li/ul/li/ul/li/a")

        for gazette_seletor in gazettes_seletor:
            date = dateparser.parse(gazette_seletor.xpath("//b/text()")
                                    .get()
                                    .split(":")[0]
                                    .strip(),
