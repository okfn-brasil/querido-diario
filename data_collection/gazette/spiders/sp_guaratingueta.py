...
class SpGuaratinguetaSpider(BaseGazetteSpider):
    ...
    def start_requests(self):
        years = [r.year for r  in rrule(freq=YEARLY, dtstart=self.start_date, until=datetime.date.today())]
        for year in years:
    ...
    def parse(self, response):
        texts = response.xpath('//*[@id="content"]/article/div[1]/ul/li/a//text()').getall()
        gazette_urls = response.xpath('//*[@id="content"]/article/div[1]/ul/li/a/@href').getall()
        for gazette_url, text in zip(gazette_urls,texts):
            ...
            is_extra_edition = ("EXTRAORDINÁRIA" in text) or \
                                ("ESPECIAL" in text) or \
                                ("GABARITO" in text) or ("EXTRAORDINÁRIO" in text)
            yield Gazette(
                date=gazette_date,
                file_urls=file_urls,
