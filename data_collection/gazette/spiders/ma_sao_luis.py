...
class MaSaoLuisSpider(BaseGazetteSpider):
    ...
    # GWT RCP Payloads
    RPC_CONFIG_PAYLOAD = (
        "7|0|7|http://sistemas.semad.saoluis.ma.gov.br/easysearch/easysearch_searchview/|"
    ...
    def bootstrap(self, response):
        yield self._send_gwt_request(
            self.RPC_BOOTSTRAP_PAYLOAD, meta=response.meta, callback=self.search_by_date
        )

    def search_by_date(self, response):
        date = response.meta["date"]
        date_formatted = "%d»%s»%d" % (date.year, self.MONTHS[date.month - 1], date.day)
    ...
    def group_results(self, content):
        ...
        for idx in idxs:
            ...
            editions.append({"extra": extra, "id": id, "text": text, "number": number})
