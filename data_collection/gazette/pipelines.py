...
class SQLDatabasePipeline:
    ...
    def _generate_entity_spider_map(self):
        ...
        for spider_class in classes:
            ...
            website_url = getattr(spider_class, "user_website", None)
            public_entity_id = getattr(spider_class, "PUBLIC_ENTITY_ID", None)
            date_from = getattr(spider_class, "start_date", None)
            if all((spider_name, website_url, public_entity_id, date_from)):
                mapping.append((spider_name, website_url, public_entity_id, date_from))
        return mapping

    def open_spider(self, spider):
        if self.database_url is not None:
            entity_spider_map = self._generate_entity_spider_map()
            engine = initialize_database(self.database_url, entity_spider_map)
    ...
    def process_item(self, item, spider):
        ...
        for file_info in item.get("files", []):
            ...
            gazette_item["file_extension"] = Path(file_info["path"]).suffix[1:]
            gazette_item["scraped_file_path"] = file_info["path"]
            gazette_item["scraped_url"] = file_info["url"]
            ...
            try:
                ...
                spider.logger.warning(
                    ...
                    f"Scraped File Checksum: {gazette_item['scraped_file_checksum']}. "
                    f"Details: {exc.args}"
                )
                session.rollback()
